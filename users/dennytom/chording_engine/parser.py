...
def add_includes(data):
    ...
    if len(data["extra_dependencies"]) > 0:
        for dependecy in data["extra_dependencies"]:
            output_buffer += f'#include "{dependecy}' + '"\n'
...
def parse_chords(data):
    ...
    keyboard_part_2 += (
        f'    {reduce(comma_separator, [f"&chord_{str(i)}" for i in range(number_of_chords)])}'
        + "\n"
    )
    keyboard_part_2 += "};\n"
    keyboard_part_2 += "\n"
...
def parse_strings_for_chords(data):
    ...
    if len(strings) > 0:
        keyboard_part_1 += f'    {reduce(comma_separator, [f"string_{str(i)}" for i in range(len(strings))])}'
    keyboard_part_1 += "\n};\n"
